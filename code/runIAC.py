...
def degree_categories(G, prange):
    ...
    sorted_d = chunks(sorted(d.iteritems(), key = lambda d: d[degree]), len(prange))
...
def findCCs_size_distribution(G, Ep, T):
    ...
    histogram = [(s, len(list(group))) for (s, group) in groupby(sortedCCs, key = lambda sortedCCs: sortedCCs[size])]
